...
public class PoliceDispatchSimulation {
	...
	private MinHeap triageQueue; //which event for police to respond to based on type/report time
	private MinHeap eventQueue; //declares sequence of events that are yet to come
	...
	public void run() {
		int time = 0;
		Incident[] inResponse = {null, null, null, null};

		while(this.eventQueue.size() > 0){

			HeapElement poll = this.eventQueue.peek();
			time = poll.value.getReportTime();

			if(poll.value.getReportTime() == time){
				fillTriageQueue(time);
			}

			policeResponse(inResponse, time);
		}

		while(this.triageQueue.size() > 0){
			time = timeMin(inResponse);
			policeResponse(inResponse, time);
		}

		for(int i = 0; i < inResponse.length; i++){
			logResolution(inResponse[i].getResolutionTime(), inResponse[i]);
			this.availableUnits[i] = true;
			inResponse[i] = null;
		}
	}

	private void fillTriageQueue(Integer time){

		if(this.eventQueue.size() > 0 ) {

			HeapElement inc = this.eventQueue.peek();

			while (inc.value.getReportTime() == time) {

				HeapElement adder = this.eventQueue.poll();
				logReport(time, adder.value);
				this.triageQueue.add(adder, 10000 * (3 - adder.value.getIncidentType().ordinal()) + adder.value.getReportTime() );
				if(this.eventQueue.size() == 0){
					break;
				} else {
					inc = this.eventQueue.peek();
				}
	...
	private Incident[] policeResponse(Incident[] inResponse, Integer time){
		for(int i = 0; i < inResponse.length; i++){
			if(inResponse[i] != null) {
				if (inResponse[i].getResolutionTime() <= time) {
					logResolution(inResponse[i].getResolutionTime(), inResponse[i]);
					this.availableUnits[i] = true;
					inResponse[i] = null;
				}
		...
		while(nextAvailableUnit() != -1 && triageQueue.size() > 0){
			HeapElement response = triageQueue.poll();
			response.value.dispatch(time, this.nextAvailableUnit());
			logDispatch(time, response.value);
			inResponse[this.nextAvailableUnit()] = response.value;
			markUnitUnavailable(this.nextAvailableUnit());
		}

		return inResponse;
	}

	private int timeMin(Incident[] inResponse){
		int min = 2147483647;
		for(int i = 0; i < inResponse.length; i++){
			if(min > inResponse[i].getResolutionTime() && inResponse[i].getResolutionTime() > 0){
				min = inResponse[i].getResolutionTime();
			}
		}
